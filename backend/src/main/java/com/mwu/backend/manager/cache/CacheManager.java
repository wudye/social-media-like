...
//    TopK 是一个热点 Key 检测器，使用 HeavyKeeper 算法初始化。它监控访问次数最多的前 100 个 Key，并通过参数控制哈希表的宽度、深度、衰减系数等：
...
//    类中定义了一个本地缓存 localCache，通过 Caffeine 构建，设置了最大容量为 1000 条数据，并配置了 5 分钟的过期时间：
    @Bean
